# nf-core/crisprquant: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/crisprquant/usage](https://nf-co.re/crisprquant/usage)
...
```bash
nextflow run nf-core/crisprquant --input samplesheet.csv -profile docker
```
...
```bash
nextflow pull nf-core/crisprquant
```
...
First, go to the [nf-core/crisprquant releases page](https://github.com/nf-core/crisprquant/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/crisprquant`](https://hub.docker.com/r/nfcore/crisprquant/)
* `singularity`
